...
abstract class ListEditor<R : Any, E : Editor<R>>(
    ...
    private val editorClass = getTypeArgument(ListEditor::class, 1).jvmErasure

    /**
     * All child editors of this list editors
     */
     ...
     */
    val results: ReactiveList<R?> = editors.map { it.result }.values()

    final override val result: ReactiveValue<List<R>?> = binding<List<R>?>(results) { results.now.takeIfNoNulls() }
    ...
    /**
     * Return the [Context] used for children of this list editor.

 ...
 */
fun <E : Editor<*>, R : Editor<*>, Ctx> ExpanderDelegate<E, Ctx>.map(f: (E) -> R) = object : ExpanderDelegate<R, Ctx> {
    override fun expand(text: String, context: Ctx): R? = this@map.expand(text, context)?.let(f)

    override fun expand(item: Any, context: Ctx): R? = this@map.expand(item, context)?.let(f)
}
